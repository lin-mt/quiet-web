...
type SchemaItemProps = {
  ...
  updateRequiredProperty?: (path: number[], requiredProperty: string, removed: boolean) => void;
};
...
function SchemaItem(props: SchemaItemProps) {
  ...
  const [nodeDepth, setNodeDepth] = useState(props.nodeDepth ? props.nodeDepth : 0);
  const [namePath, setNamePath] = useState<number[]>(props.namePath ? props.namePath : []);
  ...
  return (
    <>
      ...
      <Row align={'middle'} style={{ paddingBottom: 10 }}>
        <Col flex={`${(nodeDepth + 1) * 24}px`} style={{ marginLeft: nodeDepth * 5 }}>
          <Row justify={'end'}>
            {schema.type === 'object' && (
              <Button
                type={'text'}
        ...
        <Col flex={'auto'} style={{ marginLeft: 5 }}>
          <Input
            ...
            onChange={(description) => setSchemaDescription(description.target.value)}
          />
        </Col>
        ...
        <Form
          ...
          onValuesChange={(_, allValues) => {
            if (editorRef.current) {
              editorRef.current.setValue(JSON.stringify({ ...formSchema, ...allValues }, null, 2));
            }
          }}
        >
            ...
            <Row justify={'start'} align={'middle'} style={{ marginBottom: 13 }}>
              ...
              <Col span={8}>
                <Form.Item noStyle name={'default'}>
                  {isString && <Input style={{ width: '100%' }} placeholder={'请输入默认值'} />}
                  {(isNumber || isInteger) && (
              ...
              <Col span={8}>
                <Form.Item noStyle name={'minLength'}>
                  <InputNumber
                    ...
                    formatter={(value) => (value ? `${Math.floor(Math.max(value, 0))}` : '')}
                    placeholder={'请输入最小长度'}
                  />
                </Form.Item>
              </Col>
              ...
              <Row justify={'start'} align={'middle'} style={{ marginBottom: 13 }}>
                ...
                <Col span={8}>
                  <Form.Item noStyle name={'minimum'}>
                    <InputNumber style={{ width: '100%' }} placeholder={'请输入最小值'} />
                  </Form.Item>
                </Col>
                <Col span={4} style={{ textAlign: 'right' }}>
                  最大值：
                ...
                <Col span={8}>
                  <Form.Item noStyle name={'maximum'}>
                    <InputNumber style={{ width: '100%' }} placeholder={'请输入最大值'} />
                  </Form.Item>
                </Col>
              </Row>
              <Row justify={'start'} align={'middle'} style={{ marginBottom: 13 }}>
                ...
                <Col span={8}>
                  <Form.Item noStyle name={'exclusiveMinimum'}>
                    <InputNumber style={{ width: '100%' }} placeholder={'请输入排他最小值'} />
                  </Form.Item>
                </Col>
                <Col span={4} style={{ textAlign: 'right' }}>
                  排他最大值：
                ...
                <Col span={8}>
                  <Form.Item noStyle name={'exclusiveMaximum'}>
                    <InputNumber style={{ width: '100%' }} placeholder={'请输入排他最大值'} />
                  </Form.Item>
                </Col>
              </Row>
            </>
                    ...
                    <>
                      <Row>
                        {fields.map(({ key, name, ...restField }) => (
                          <Col span={12} key={key}>
                            <Row justify={'start'} align={'middle'} style={{ marginBottom: 6 }}>
                              <Col flex={'auto'}>
                                <Form.Item
                                  {...restField}
                                  noStyle
